...
describe('GET /api/concerts', () => {
  before(async () => {
    const dummyConcert = new Concert({ performer: 'Zibi Wodecki', genre: 'Classic', price: 30, day: 3, image: 'wodecky.png' });
    await dummyConcert.save();
  });

  after(async () => {
    await Concert.deleteMany({ performer: 'Zibi Wodecki', genre: 'Classic', price: 30, day: 3, image: 'wodecky.png'  });
  });

  it('getPerformer should return performer', async () => {
    const res = await request(server).get('/api/concerts/performer/Zibi Wodecki');
  ...
  it('getPrice should return performer within price range ', async () => {
  ...
  it('getPrice should not return performer within price range ', async () => {
